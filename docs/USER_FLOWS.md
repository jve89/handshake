...
- **Viewer (token)** — anyone with an **inbox token link** can view submissions (read-only)

---

## Navigation Context (3-Layer UI)

- **Layer 1 — Inbox / Outbox** (incoming vs outgoing)
- **Layer 2 — Folders** (UI-only in MVP; optional)
- **Layer 3 — Handshakes** (list + detail)

> Users may **skip Layer 2** via **“See all handshakes”**, which jumps directly to Layer 3.
...
- [x] **Archive** handshake (dashboard filter **Active / Archived / All**; **archived remain public**)
- [ ] **Duplicate** handshake (planned)

**Immutability:** **Link ID (slug) cannot change after creation** → server returns `400 slug_immutable`.  
**Plan limits:** Free plan allows **1 active** handshake → creating/unarchiving beyond the limit returns `403 plan_limit_reached { maxActive: 1 }`.
