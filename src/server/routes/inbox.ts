...
router.get("/submissions", async (req: Request, res: Response) => {
  ...
  return res
    .status(403)
    .json({ error: "Token scope not supported for this endpoint" });
});

/** GET /api/inbox/handshakes/:handshakeId/submissions */
router.get(
  "/handshakes/:handshakeId/submissions",
  async (req: Request, res: Response) => {
    ...
    if (scope.handshakeId !== handshakeId)
      return res
        .status(403)
        .json({ error: "Token not scoped to this handshake" });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (scope.handshakeId) {
      ...
      if (hid !== scope.handshakeId)
        return res
          .status(403)
          .json({ error: "Token not scoped for this submission" });
    }
