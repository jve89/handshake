...
function group(rows: SubmissionRow[]): Submission[] {
  ...
  for (const row of rows) {
    ...
    if (!s) {
      s = { submission_id: row.submission_id, submitted_at: row.submitted_at, responses: [] };
      map.set(row.submission_id, s);
    }
    s.responses.push({ request_id: row.request_id, label: row.label, value: row.value });
  }
  return Array.from(map.values());
}

export async function listByHandshake(handshakeId: number, limit = 100, offset = 0): Promise<Submission[]> {
...
export async function getSubmission(submissionId: number): Promise<Submission | null> {
  const q = await db.query<SubmissionRow>(
    `
...
export async function getSubmissionHandshakeId(submissionId: number): Promise<number | null> {
  const r = await db.query<{ handshake_id: number }>(`SELECT handshake_id FROM submissions WHERE id = $1`, [submissionId]);
